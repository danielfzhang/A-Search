...
while True:
    for event in pygame.event.get():#detect keyboard, mouse inputs
        if event.type == QUIT: exit()
            ...
            if pressed_mouse[0]:
                leftClick=True
            elif pressed_mouse[2]: rightClick=True
        elif event.type ==  MOUSEBUTTONUP:
            leftClick=False
            rightClick=False
        elif event.type == KEYDOWN:
            if event.key == K_RETURN and not monsterMove:#generage monster
                monsterMove=True
                t1=threading.Thread(target=geneMonster,args=(Start,))
                t1.setDaemon(True)
                t1.start()
    #functions after something is inputted
    if leftClick or rightClick:#click mouse to move map, set blocks or remove blocks
